...
SITL_SIM_DIR=${PX4_SRC_DIR}/Tools/simulation
...
if [ "${SITL_TYPE}" = "gazebo" ]; then
    ...
    if [ ! -d ./PX4-Autopilot ]; then
        tar -zxvf ${HOME}/builds/${SITL_TYPE}.tar ./
    fi
...
## CASE A-2. gazebo-classic SIMULATION [ERROR]
    ...
    if [ ! -d ./PX4-Autopilot ]; then
        tar -zxvf ${HOME}/builds/${SITL_TYPE}-${SITL_AIRFRAME}.tar ./
    fi

    COMMENT_START=$(grep -wn 'if [ "\$debugger" == "lldb" ]; then' ${SITL_SIM_DIR}/gazebo-classic/sitl_run.sh | cut -d: -f1)
    COMMENT_END=$(grep -wn 'kill -9 \$SIM_PID' ${SITL_SIM_DIR}/gazebo-classic/sitl_run.sh | cut -d: -f1)
...
## CASE A-3. NO SIMULATION DESIGNATED [ERROR]
    ...
    echo -e "[ERROR]\tNO valid \${SITL_TYPE} DESIGNATED"
    exit 1
fi
...
if [ "${DEBUG_MODE}" -eq "1" ]; then
    ...
    if [ "${EXPORT_ENV}" -eq "1" ]; then
        ...
        #### CASE B-1-1-3: NO SIMULATION DESIGNATED [ERROR]
            ...
            echo -e "ERROR\t[SITL]NO valid \${SITL_TYPE} DESIGNATED"
            exit 1
            
        fi

        echo -e"INFO\t[SITL] ENVIRONMENT VARS FOR SITL WILL BE EXPORTED"
    ...
    ### CASE B-1-2: NO LEAVE THEM CLEAN
        ...
        echo -e "INFO\t [SITL] ENVIRONMENT VARS WILL NOT BE SET"
    fi
